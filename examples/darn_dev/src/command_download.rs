...
use daml::lf::element::{DamlAbsoluteTyCon, DamlElementVisitor, DamlNonLocalTyCon};
use daml::lf::{DamlLfArchivePayload, PackageInfo, LanguageVersion};
use daml::lf::{
...
pub async fn download(
    ...
    let extended_package_list = PackageInfo::extract_from_dar(&working_dar)?;
    let extended_package_info: HashMap<&str, &PackageInfo> =
    ...
    // let eps: HashSet<PackageInfo> = working_dar.apply(|arc| {
...
fn write_dar(
    ...
    main_package_id: &str,
    package_info: &HashMap<&str, &PackageInfo>,
    output_dir: &str,
) -> Result<()> {
    ...
    let dependency_info: Vec<&PackageInfo> = package_info
        .iter()
        .filter_map(|(&k, &v)| {
...
fn make_dalf_path(root_dir: &str, info: &PackageInfo) -> String {
    format!("{}/{}-{}.dalf", root_dir, info.package_name, info.package_id)
}

// TODO lots of cloning here

fn make_final_packages(_packages: Vec<DamlPackage>) -> Result<HashMap<String, Vec<u8>>> {
    todo!()
    // packages
    //     .into_iter()
    //     .map(|d| {
...
//         refs: HashSet::<PackageInfo>::default(),
//     };
...
impl DamlElementVisitor for PackageDependencyVisitor {
    fn pre_visit_non_local_tycon(&mut self, non_local_data_ref: &DamlNonLocalTyCon<'_>) {
        self.refs.insert(PackageInfo {
            ...
            version: None,
            language_version: LanguageVersion::V1_DEV,
        });
    }
