...
pub async fn download(uri: &str, token_key_path: Option<&str>, main_package_name: &str) -> Result<()> {
    ...
    let included_package: HashMap<&String, &String> =
        once((&main_package_info.package_id, &main_package_info.package_name))
            .chain(dependencies.iter().map(|d| (&d.package_id, &d.package_name)))
            .collect();

    // 3: we have to filter and rename each DamlLfArchive with the name we have just extracted from the package
    let renamed: HashMap<&String, DamlLfArchive> = once(working_dar.main)
        .chain(working_dar.dependencies)
        .filter_map(|mut a| {
            included_package.get_key_value(&a.hash).map(|(&k, &v)| {
                a.name = v.clone();
                (k, a)
            })
        })
...
fn make_final_dar(mut all_packages: HashMap<&String, DamlLfArchive>, package_id: &String) -> DarFile {
