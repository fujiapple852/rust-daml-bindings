use crate::common::ping_pong::{
    ...
    new_static_sandbox, TestResult2, ALICE_PARTY, APPLICATION_ID_PREFIX, BOB_PARTY, COMMAND_ID_PREFIX, ERR_STR,
...
use daml_grpc::service::{DamlGetCompletionStreamResult, DamlSubmitRequestErrorCodes, GrpcApiError};
use futures::prelude::*;
...
/// expect.
#[tokio::test]
async fn test_command_submission_and_completion() -> TestResult2 {
    ...
    let command_id = match command_id {
        Ok(cid) => cid,
        Err(GrpcApiError::ApiError {
            error: DamlSubmitRequestErrorCodes::Unauthenticated,
            ..
        }) => panic!(),
        _ => panic!(),
    };
    ...
    // TODO
    // let completion_stream = match completion_stream {
    //     Ok(o) => o,
    //     Err(ee @ GrpcApiError::ApiError { error: DamlGetCompletionStreamErrorCodes::Unauthenticated, .. }) => {
    ...
    let completions: Vec<DamlCompletion> = completion_stream
        ...
        .take(1)
        .map(|f: DamlGetCompletionStreamResult<DamlCompletionResponse>| {
            f.map(|resp| resp.take_completions().swap_remove(0))
        })
        .try_collect()
        .await?;
    let completion = completions.first().ok_or(anyhow!(ERR_STR))?;
...
fn is_skippable_completion(f: &DamlGetCompletionStreamResult<DamlCompletionResponse>) -> impl Future<Output = bool> {
