...
fn test_get_transaction_trees() -> TestResult {
    ...
    test_exercise_pong_choice(&ledger_client, &application_id, &workflow_id, &create_test_uuid(COMMAND_ID_PREFIX))?;
    ...
    assert_eq!(&create_test_pp_id(PING_ENTITY_NAME), ping_exercised_event.template_id());
...
fn test_get_transaction_by_event_id() -> TestResult {
    ...
    test_create_ping_contract(&ledger_client, &application_id, &workflow_id, &command_id, 0)?;
    ...
    assert_eq!(PINGPONG_PACKAGE_ID, event.template_id().package_id());
    ...
    assert_eq!(&create_test_pp_id(PING_ENTITY_NAME), event.create_arguments().record_id().as_ref().ok_or(ERR_STR)?);
    Ok(())
}
