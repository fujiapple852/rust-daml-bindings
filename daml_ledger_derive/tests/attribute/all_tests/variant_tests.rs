...
fn test_circle_variant() -> TestResult {
    ...
    let transaction = alice_executor.execute_for_transaction_sync(DamlCommand::Create(create_command))?;
    ...
    let exercise_transaction = alice_executor.execute_for_transaction_sync(DamlCommand::Exercise(exercise_command))?;
