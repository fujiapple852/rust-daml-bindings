...
fn test_using_enum() -> TestResult {
    ...
    let create_car_command = update_create_command_package_id_for_testing(&client, create_car_command)?;
    let create_car_result = alice_executor.execute_for_transaction_sync(DamlCommand::Create(create_car_command))?;
    ...
    let repaint_result = alice_executor.execute_for_transaction_sync(DamlCommand::Exercise(exercise_command))?;
