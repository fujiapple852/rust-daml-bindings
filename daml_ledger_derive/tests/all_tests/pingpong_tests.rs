...
use daml_ledger_macro::daml_path;
...
fn test_ledger_create() -> TestResult {
    ...
    let ping_result = test_submit_command(&ledger_client, "Alice", create_ping_command)?;
...
fn test_ledger_create_and_exercise_with_nested() -> TestResult {
    ...
    let exercise_command = ping_contract.set_from_user_data(5, UserData::new("foo", 2));
    let ping_reset_result = test_submit_command(&ledger_client, "Alice", exercise_command)?;
