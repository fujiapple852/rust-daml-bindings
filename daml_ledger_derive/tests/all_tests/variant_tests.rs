...
use crate::domain::all_variant_types::*;
use crate::domain::variant_example::*;
...
#[test]
fn test_circle_variant() -> TestResult {
    ...
    let create_command = circle_template.create();
    let transaction = test_submit_command(&ledger_client, "Alice", create_command)?;
    ...
    let exercise_command = circle_contract.replace_circle(Circle::new(1.223_000_000_1, Color::Custom(vec![1, 2, 3])));
    let exercise_transaction = test_submit_command(&ledger_client, "Alice", exercise_command)?;
...
#[test]
fn test_all_variant_types() -> TestResult {
    let _lock = SANDBOX_LOCK.lock()?;
    let ledger_client = new_static_sandbox()?;
    ...
    let create_command = variant_template.create();
    let transaction = test_submit_command(&ledger_client, "Alice", create_command)?;
    let event: DamlEvent = transaction.take_events().swap_remove(0);
