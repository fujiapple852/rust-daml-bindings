...
impl<'a> From<&'a DefDataType> for DamlDataPayload<'a> {
    fn from(def_data_type: &'a DefDataType) -> Self {
        let name = leaf_name(def_data_type.name.as_ref().expect("DefDataType.name"));
        match def_data_type.data_cons.as_ref().expect("DefDataType.data_cons") {
            ...
            DataCons::Enum(constructors) =>
                Self::new_enum(EnumPayload::new(name, constructors.constructors.as_slice())),
        }
    }
}
...
impl<'a> DamlTemplatePayload<'a> {
    pub fn new(name: &'a str, choices: Vec<DamlChoicePayload<'a>>) -> Self {
        Self {
            name,
            choices,
        }
...
impl<'a> From<&'a DefTemplate> for DamlTemplatePayload<'a> {
    fn from(def_template: &'a DefTemplate) -> Self {
        Self::new(
            leaf_name(def_template.tycon.as_ref().expect("DefTemplate.tycon")),
...
impl<'a> DamlChoicePayload<'a> {
    pub fn new(name: &'a str, argument_type: DamlTypePayload<'a>, return_type: DamlTypePayload<'a>) -> Self {
        Self {
            name,
            argument_type,
            return_type,
...
impl<'a> RecordPayload<'a> {
    pub fn new(name: &'a str, fields: Vec<DamlFieldPayload<'a>>) -> Self {
        Self {
            name,
            fields,
        }
