...
use crate::element::{
    DamlAbsoluteDataRef, DamlChoice, DamlDataRef, DamlEnum, DamlField, DamlLocalDataRef, DamlRecord, DamlTemplate,
    DamlType, DamlVariant,
};

impl<'a> From<&'a AttrRecord> for DamlRecord<'a> {
    fn from(attr_record: &'a AttrRecord) -> Self {
        let fields: Vec<DamlField> = attr_record.fields.iter().map(DamlField::from).collect();
        DamlRecord::new(&attr_record.name, fields)
    }
}
...
impl<'a> From<&'a AttrVariant> for DamlVariant<'a> {
    fn from(attr_variant: &'a AttrVariant) -> Self {
        ...
        DamlVariant::new(&attr_variant.name, variant_fields)
    }
}
